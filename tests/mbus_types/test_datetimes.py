...
@pytest.mark.parametrize(
    ...
    [
        ...
        (24, 0, 0, pytest.raises(ValueError, match="hour must be in 0..23")),
    ],
)
def test_time_init(
    hour: int,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            1,
            1,
            pytest.raises(ValueError, match="year 0 is out of range"),
        ),
        (
            1,
            0,
            ...
            1,
            1,
            pytest.raises(ValueError, match="month must be in 1..12"),
        ),
        (
            1,
            1,
        ...
        (1, 1, 1, 0, 0, 0, does_not_raise()),
        (1, 1, 1, 1, 1, 1, does_not_raise()),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "3B 17 6A 25",
            datetime(year=2019, month=5, day=10, hour=23, minute=59, second=0),
        ),
        (
            "3B 17 6A 25 3B FF",
            datetime(year=2019, month=5, day=10, hour=23, minute=59, second=59),
...
def test_parse_datetime_byte_mismatch():
    ...
    answer = datetime(year=2019, month=5, day=10, hour=23, minute=59, second=0)
...
def test_datetime_to_strings():
    ...
    assert (
        datetime_.to_datetime_no_sec()
        == f"{year}-{month}-{day}T{hour}:{minute}"
    )
